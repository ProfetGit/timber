...
**As a server plugin:** the same pack also comes as a plugin for Paper, Purpur, Spigot and Bukkit. Put `Timber-1.2.0-plugin.jar` in the `plugins` folder and restart the server. On Paper and Purpur the plugin loads the pack itself; on Spigot and Bukkit it copies the pack into `world/datapacks/` and reloads. Use either the plugin or the zip, not both.
...
Installed as a plugin? Run step 1, then remove the jar from the `plugins` folder. On Spigot and Bukkit, also delete `world/datapacks/vanilla_timber.zip`. Restart the server.
